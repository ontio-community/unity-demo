...
namespace OntologyCSharpSDK.Core
{
    ...
    public class ScriptBuilder : IDisposable
    {
        ...
        public ScriptBuilder Write(byte[] data)
        {
            writer.Write(data);
            return this;
        }
        ...
        public static byte[] GetParamBytes(string str)
        {
            ...
            {
                var hex = Helper.HexString2Bytes(str.Substring(7));
                return hex;
            }
            else if (str.IndexOf("([])") == 0)
            {
                var hex = Helper.HexString2Bytes(str.Substring(4));
                return hex;
            }
            //(address) or(addr)开头，表示是一个地址，转换为脚本hash
            else if (str.IndexOf("(address)") == 0)
            {
                ...
                var hex = Helper.GetPublicKeyHashFromAddress(addr);
                return hex;
            }
            ...
            {
                var hex = Helper.HexString2Bytes(str.Substring(12));
                return (hex.Reverse().ToArray());
            }
            else if (str.IndexOf("(hexint)") == 0)
            {
                var hex = Helper.HexString2Bytes(str.Substring(8));
                return (hex.Reverse().ToArray());
            }
            else if (str.IndexOf("(hex)") == 0)
            {
                var hex = Helper.HexString2Bytes(str.Substring(5));
                return (hex.Reverse().ToArray());
            }
            //(int256) or (hex256) 开头,表示是一个定长的256位 16进制大整数
            ...
            {
                var hex = Helper.HexString2Bytes(str.Substring(9));
                if (hex.Length != 32)
                    throw new Exception("error lenght");
                return (hex.Reverse().ToArray());
            }
